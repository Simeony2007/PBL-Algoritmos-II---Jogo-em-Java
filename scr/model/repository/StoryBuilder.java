...
public class StoryBuilder {
    ...
    public static ArrayList<Npc> GetNpcs(ArrayList<Npc> npcs){
    ...
    public static Chapter BuildChapter1(Chapter chapter){
        chapter.setTitle("Capítulo 1 - A Entrada do Submundo");
        ...
        scene1.addOption// FINALIZAR OS OPTIONS!("Jurei protegê-la até o fim dos meus dias, e nem mesmo a morte tem o direito de quebrar nosso laço.", scene2, 0, 2, 0, 0, 0);
        scene1.addOption("Ficar na terra sem Eurídice é uma sentença pior que o próprio Tártaro. Eu não tenho nada mais a perder.", scene2, 0, 0, 0, 2, 0);
        scene1.addOption("A tirania das Moiras não vai roubar minha esposa. Eu vou descer ao reino de Hades e tomar o que é meu de volta.", scene2, 0, 0, 2, 0, 5);
        ...
        scene2.addOption("Uma melodia suave e envolvente que evoca o repouso da natureza, abrandando a ferocidade da besta até fazê-la adormecer.", scene2a, 0, 1, 0, 0, 0);
        scene2.addOption("Um canto lúgubre sobre a dor da perda, conectando-se com a própria essência sombria do Submundo e fazendo o guardião lamentar em vez de atacar.", scene2b, 0, 0, 0, 1, 0);
        scene2.addOption("Acordes retumbantes e agressivos que simulam o rugido da guerra, impondo respeito e subjugando o cão pela força e imponência do som.", scene2c, 0, 0, 1, 0, 0);
        chapter.addScene(scene2);

        // Cenas 2a, 2b e 2c - Resultados das escolhas com o Cérbero
        scene2a.addDialogue("Os acordes suaves da lira ecoam como a brisa entre as folhas, fazendo o olhar furioso de Cérbero suavizar-se até que suas três cabeças desabem no chão de pedra em um sono profundo, permitindo que Orfeu se esgueire em direção às sombras do rio Aqueronte murmurando que o amor os guiará de volta.");
        scene2a.setNextScene(scene3);

        chapter.addScene(scene2a);
        scene2b.addDialogue("As notas graves e melancólicas carregadas do peso de um luto insuportável fazem Cérbero soltar ganidos lúgubres pelas três bocas, chorando em sintonia com a dor do bardo enquanto este contorna o guardião cabisbaixo, determinado a transformar a própria tragédia no caminho para resgatar sua amada.");
        scene2b.setNextScene(scene3);
        chapter.addScene(scene2b);

        scene2c.addDialogue("O dedilhado agressivo e vibrante ressoa como o estalo do trovão, intimidando a besta colossal que dá passos para trás e se curva em submissão involuntária, abrindo passagem para Orfeu avançar com passos firmes em direção às profundezas do reino dos mortos.");
        scene2c.setNextScene(scene3);
        chapter.addScene(scene2c);
